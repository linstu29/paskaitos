...
calculateButton.addEventListener('click', function() {
    ...
    let priceCount = document.getElementById('price-count').valueAsNumber

    let kepyklaTotal = employeeCount * loavesPerEmployee
    let arPavyks = kepyklaTotal >= orderCount
    let priceTotal = kepyklaTotal * priceCount;
    ...
    results.innerHTML += `<p><strong>Koks pelnas?</strong> ${priceTotal} €</p>`
})
...
document.getElementById('price-count').addEventListener('keyup', function(event) {
    let inputValue = event.target.valueAsNumber
    if(inputValue < 0) {
        event.target.classList.add('error')
        event.target.nextElementSibling.classList.add('show')
...
document.getElementById('reset').addEventListener('click', function() {
    ...
    document.getElementById('price-count').valueAsNumber = 0
